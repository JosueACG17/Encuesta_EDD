...
const countries = [
  ...
  'Hong Kong', 'Hungary', 'Iceland', 'India', 'Indonesia', 'Iraq', 'Ireland', 'Israel', 'Italy', 'Jamaica',
  'Japan', 'Jordan', 'Kazakhstan', 'Kenya', 'Kosovo', 'Kuwait', 'Kyrgyzstan', 'Latvia', 'Lebanon',
  'Liechtenstein', 'Lithuania', 'Luxembourg', 'Malaysia', 'Maldives', 'Malta', 'Mexico', 'Moldova',
  'Monaco', 'Montenegro', 'Morocco', 'Nepal', 'Netherlands', 'New Zealand', 'Nigeria', 'North Macedonia',
  'Norway', 'Oman', 'Pakistan', 'Panama', 'Paraguay', 'Peru', 'Philippines', 'Poland', 'Portugal',
  'Qatar', 'Romania', 'Russia', 'San Marino', 'Serbia', 'Singapore', 'Slovakia', 'Slovenia',
  'South Africa', 'South Korea', 'Spain', 'Sri Lanka', 'Sweden', 'Switzerland', 'Syria', 'Taiwan',
  'Tajikistan', 'Thailand', 'Trinidad', 'Turkey', 'UAE', 'UK', 'USA', 'Ukraine', 'Uruguay',
...
const submitForm = async () => {
    ...
    const payload = {
      ...
      academic_level: formData.value.Academic_Level,
      country: formData.value.Country,
      avg_daily_usage_hours: Number(formData.value.Avg_Daily_Usage_Hours.toFixed(1)),
      most_used_platform: formData.value.Most_Used_Platform,
      affect_academic_performance: formData.value.Affects_Academic_Performance === 'Sí' ? 1 : 0,
      sleep_hours: Number(formData.value.Sleep_Hours_Per_Night.toFixed(1)),
      mental_health: formData.value.Mental_Health_Score,
      relationship: formData.value.Relationship_Status,
      conflicts_over_social_media: formData.value.Conflicts_Over_Social_Media,
      addicted_score: formData.value.Addicted_Score,
      deleted: 0
    }

    const { error } = await supabase.from('Social_media').insert(payload)
